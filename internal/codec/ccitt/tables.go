...
// entry is one lookup-table slot: the code length n (0 marks an invalid
// prefix) and its value: a run length for the colour tables, a mode for
// modeTable.
type entry struct {
	n   uint8
	run uint16
...
// Mode values stored in modeTable. The vertical modes are laid out so that
// the offset of a1 from b1 is run-modeV0.
const (
	modePass  = 1
	modeHoriz = 2
	modeExt   = 3
	modeV0    = 7 // VL3..VR3 are modeV0-3 .. modeV0+3
)

// Table widths: the longest white code is 9 bits (12 with the shared
// extended makeups), the longest black code 13 bits, the longest mode
// code 7 bits.
const (
	whiteBits = 12
	blackBits = 13
...
var (
	whiteTable = buildTable(whiteBits, whiteCodes, extMakeupCodes)
	blackTable = buildTable(blackBits, blackCodes, extMakeupCodes)
	modeTable  = buildTable(modeBits, modeCodes)
)

// codeDef is a T.4 code word written as in the standard's tables.
type codeDef struct {
	value uint16 // run length, or mode constant
	code  string
}

// buildTable expands the code words into a direct lookup table indexed by
// the next `bits` bits of input: every index whose prefix is a code maps
// to that code.
func buildTable(bits uint, defs ...[]codeDef) []entry {
	t := make([]entry, 1<<bits)
	for _, set := range defs {
		for _, c := range set {
			n := len(c.code)
			if n < 1 || n > 16 || uint(n) > bits {
				// Every T.4 code is 1..13 bits; anything else is a typo in
				// the tables above, caught by the table tests.
				continue
			}
			var code uint
			for _, b := range []byte(c.code) {
				code = code<<1 | uint(b-'0')
			}
			shift := bits - uint(n)
			for i := code << shift; i < (code+1)<<shift; i++ {
				t[i] = entry{n: uint8(n), run: c.value}
			}
		}
	}
	return t
...
// modeCodes are the T.4 §4.2.1.3.4 / T.6 table 4 mode codes. The EOL
// prefix 0000000 is left invalid so an EOL inside a row fails cleanly.
var modeCodes = []codeDef{
	{modePass, "0001"},
	{modeHoriz, "001"},
	{modeV0, "1"},
	{modeV0 + 1, "011"},
...
// whiteCodes are the T.4 terminating (0-63) and makeup (64-1728) codes for
// white runs.
var whiteCodes = []codeDef{
...
// blackCodes are the T.4 terminating (0-63) and makeup (64-1728) codes for
// black runs.
var blackCodes = []codeDef{
...
// extMakeupCodes are the extended makeup codes (1792-2560) shared by both
// colours (T.4 table 3b).
var extMakeupCodes = []codeDef{
